...
class TranslationMultiSimpleEpochTask(LegacyFairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--eval-bleu', default=False, help="evaluation with BLEU scores", action='store_true')
        
        parser.add_argument('--eval-bleu-args', default="{}")
        
        parser.add_argument('--eval-bleu-detok', default="space", help="detokenize before computing BLEU (e.g., 'moses')")
        
        parser.add_argument('--eval-bleu-detok-args', default="{}",help="args for building the tokenizer, if needed, as JSON string")
        
        parser.add_argument('--eval-tokenized-bleu', default=False, help="compute tokenized BLEU instead of sacrebleu", action='store_true')
        
        parser.add_argument('--eval-bleu-remove-bpe', default=None, help="remove BPE before computing BLEU")
        
        parser.add_argument('--eval-bleu-print-samples', default=False, help="print sample generations during validation", action='store_true')
    ...
    def build_generator(
        ...
        if not getattr(args, "keep_inference_langtok", False):
            ...
            #logger.info("this is me printing the tgt_langtok_spec")
            #logger.info(tgt_langtok_spec)
            #logger.info("this is me printing the generation config")
            #logger.info(self.args)
            if tgt_langtok_spec:
                tgt_lang_tok = self.data_manager.get_decoder_langtok(
                    self.args.target_lang, tgt_langtok_spec
                )
                #logger.info("this is me printing the tgt_lang_tok")
                #logger.info(tgt_lang_tok)
                extra_gen_cls_kwargs = extra_gen_cls_kwargs or {}
                extra_gen_cls_kwargs["symbols_to_strip_from_output"] = {tgt_lang_tok}

        return super().build_generator(
            models, args, seq_gen_cls, extra_gen_cls_kwargs=extra_gen_cls_kwargs
        )
    ...
    def _inference_with_bleu(self, generator, sample, model):
        ...
        for i in range(len(gen_out)):
            hyps.append(decode(gen_out[i][0]["tokens"])[10:])
            refs.append(
                decode(
