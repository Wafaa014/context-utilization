...
class FairseqMultiEncoderTransformerHub(GeneratorHubInterface):
    ...
    def __init__(self, cfg, task, models, change_tokens=False):
        ...
        # for loading the som model
        self.spm_dir = cfg.checkpoint.save_dir
        self.eval()
    ...
    def trace_forward(self, src_tensor, tgt_tensor, src_ctx_tensor, tgt_ctx_tensor):
        ...
        with torch.no_grad():
            ...
            src_ctx_tensor = src_ctx_tensor.unsqueeze(0).to(self.device)
            ...
            tgt_ctx_tensor = tgt_ctx_tensor.unsqueeze(0).to(self.device)
            ...
            model_output, encoder_out = self.models[0](
                ...
                tgt_tensor,
                src_ctx_tensor,
                src_ctx_tensor.size(-1),
                tgt_ctx_tensor,
                tgt_ctx_tensor.size(-1),
            )
    ...
    def _get_attn_weights_module(self, layer_outputs, module_name, layers_names):
        ...
        k = layer_outputs[f"models.0.{enc_dec_}.{layers_names}.{l}.{attn_module_}.k_proj"][0]
        q = layer_outputs[f"models.0.{enc_dec_}.{layers_names}.{l}.{attn_module_}.q_proj"][0]
    ...
    def _get_contributions_module(
        self, layer_inputs, layer_outputs, contrib_type, module_name, context
    ):
        if context:
            layers_names = "context_layers"
        else:
            layers_names = "layers"
        ...
        attn_w = self._get_attn_weights_module(
            layer_outputs, module_name, layers_names
        ...
        in_q = layer_inputs[f"models.0.{enc_dec_}.{layers_names}.{l}.{attn_module_}.q_proj"][0][
            0
        ].transpose(0, 1)
        in_v = layer_inputs[f"models.0.{enc_dec_}.{layers_names}.{l}.{attn_module_}.v_proj"][0][
            0
        ].transpose(0, 1)
        in_res = layer_inputs[
            f"models.0.{enc_dec_}.{layers_names}.{l}.{attn_module_}_layer_norm"
        ][0][0].transpose(0, 1)
        ##
        in_res2 = layer_inputs[f"models.0.{enc_dec_}.{layers_names}.{l}.final_layer_norm"][0][
            0
        ].transpose(0, 1)
        ...
        if pre_layer_norm == False:
            ...
            if "encoder" in enc_dec_:
                ...
                out_q_pre_ln_th = layer_inputs[
                    f"models.0.{enc_dec_}.{layers_names}.{l}.final_layer_norm"
                ][0][0].transpose(0, 1)
            else:
                if "self_attn" in attn_module_ and layers_names== "layers":
                    #print("in the if")
                    # Self-attention decoder -> encoder_attn_layer_norm
                    out_q_pre_ln_th = layer_inputs[
                        f"models.0.{enc_dec_}.{layers_names}.{l}.encoder_attn_layer_norm"
                    ][0][0].transpose(0, 1)
                else:
                    #print("in the else")
        ...
        #print(torch.dist(out_q_pre_ln_th, out_q_pre_ln).item())
    ...
    def get_contributions(
        ...
        src_ctx_tensor: torch.Tensor,
        tgt_ctx_tensor: torch.Tensor,
        contrib_type: str = "l1",
        ...
        _, _, _, layer_inputs, layer_outputs = self.trace_forward(
            src_tensor, tgt_tensor, src_ctx_tensor, tgt_ctx_tensor
        )
        ...
        for attn in self.ATTN_MODULES:
            ...
            for l in range(len(enc_dec.layers)):
                ...
                if resultant_norms.size() == torch.Size([]):
                    resultant_norms = resultant_norms.unsqueeze(0)
                contributions = self.normalize_contrib(contributions, norm_mode, resultant_norm=resultant_norms).unsqueeze(1)
                element[f"{attn}_current"].append(contributions)
            contributions_all[attn].append(element)
            if attn == "decoder.encoder_attn":
                continue
            else:
                element_context = defaultdict(list)
                for l in range(len(enc_dec.context_layers)):      
                    if contrib_type == "attn_w":
                        contributions = f_context(attn.replace(".", f".{l}."))
                        resultant_norms = None
                        contributions = contributions.sum(1)
                        if norm_mode != "sum_one":
                            print("Please change the normalization mode to sum one")
                    else:
                        contributions, resultant_norms = f_context(attn.replace(".", f".{l}."))
                    contributions = self.normalize_contrib(
                        contributions, norm_mode, resultant_norm=resultant_norms).unsqueeze(1)
                    element_context[f"{attn}_context"].append(contributions)
                contributions_all[attn].append(element_context)
        
        for k, v in contributions_all.items():
            for d in v:
                for d_k, d_v in d.items():
                    d[d_k] = torch.cat(d_v, dim=1)
    ...
    def get_contribution_rollout(
        ...
        **contrib_kwargs,
        ...
        contribs = self.get_contributions(
            src_tensor, tgt_tensor, src_ctx_tensor, tgt_ctx_tensor, contrib_type, norm_mode=norm_mode, **contrib_kwargs
        )
        
        ######### current encoder self attention
        # Compute contributions rollout encoder self-attn
        enc_self_attn_contributions_current = torch.squeeze(contribs[enc_sa][0]["encoder.self_attn_current"], dim=0)
        layers, _, _ = enc_self_attn_contributions_current.size()
        enc_self_attn_contributions_mix_current = compute_joint_attention(
            enc_self_attn_contributions_current
        )
        relevances_enc_self_attn_current = enc_self_attn_contributions_mix_current[-1]

        ####### context encoder self attention
        enc_self_attn_contributions_context = torch.squeeze(contribs[enc_sa][1]["encoder.self_attn_context"], dim=0)
        layers, _, _ = enc_self_attn_contributions_context.size()
        enc_self_attn_contributions_mix_context = compute_joint_attention(
            enc_self_attn_contributions_context
        )
        relevances_enc_self_attn_context = enc_self_attn_contributions_mix_context[-1]
       
        ########## context decoder self attention
        dec_self_attn_contributions_context = torch.squeeze(contribs[dec_sa][1]["decoder.self_attn_context"], dim=0)
        layers, _, _ = dec_self_attn_contributions_context.size()
        dec_self_attn_contributions_mix_context = compute_joint_attention(
            dec_self_attn_contributions_context
        )
        relevances_dec_self_attn_context = dec_self_attn_contributions_mix_context[-1]
        
        # total encoders
        total_size = relevances_enc_self_attn_current.size(0) + relevances_enc_self_attn_context.size(0) + relevances_dec_self_attn_context.size(0) 
        total_encoders = torch.zeros(total_size, total_size)
        total_encoders[:relevances_enc_self_attn_context.size(0),:relevances_enc_self_attn_context.size(0)] = relevances_enc_self_attn_context
        total_encoders[relevances_enc_self_attn_context.size(0):relevances_enc_self_attn_context.size(0) + 
            relevances_enc_self_attn_current.size(0) , relevances_enc_self_attn_context.size(0): 
            relevances_enc_self_attn_context.size(0) + relevances_enc_self_attn_current.size(0)] = relevances_enc_self_attn_current 
        total_encoders[relevances_enc_self_attn_context.size(0) + relevances_enc_self_attn_current.size(0):, 
            relevances_enc_self_attn_context.size(0) + relevances_enc_self_attn_current.size(0):] = relevances_dec_self_attn_context

        # Compute joint cross + self attention
        self_dec_contributions = torch.squeeze(contribs[dec_sa][0]["decoder.self_attn_current"], dim=0)
        cross_contributions = torch.squeeze(contribs[dec_ed][0]["decoder.encoder_attn_current"], dim=0)
        ...
        total_encoders = total_encoders.to(joint_self_cross_contributions.device) 
        contributions_full_rollout = rollout(joint_self_cross_contributions, total_encoders)
